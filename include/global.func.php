...
function gamevars_init($gamevars){
	if(!is_array($gamevars)){$gamevars = Array();}
	foreach(Array('sanmaact','sanmadead') as $val){
		if(!isset($gamevars[$val])){$gamevars[$val] = 0;}
	}
	return $gamevars;	
}
...
function load_gameinfo() {
	...
	$hack = $gameinfo['hack'];
	$gamevars = json_decode($gameinfo['gamevars'],true);
	$combonum = $gameinfo['combonum'];
	$gametype = $gameinfo['gametype'];
	$gameotherinfo = $gameinfo['gameotherinfo'];
	$is_solo = $gameinfo['is_solo'];
	return;
}
...
function save_gameinfo() {
	...
	$gameinfo['gameotherinfo'] = $gameotherinfo;
	$gameinfo['is_solo'] = $is_solo;
...
function getteamchat($last,$team='',$limit=4) {
	global $db,$tablepre,$chatlimit,$chatinfo,$plsinfo;
	$result = $db->query("SELECT * FROM {$tablepre}chat WHERE cid>'$last' AND type='1' AND recv='$team' ORDER BY cid desc LIMIT 4");
	$chatdata = Array('lastcid' => $last, 'msg' => '');
	if(!$db->num_rows($result)){$chatdata = array('lastcid' => $last, 'msg' => '');return $chatdata;}
	...
	while($chat = $db->fetch_array($result)) {
		...
		if($chatdata['lastcid'] < $chat['cid']){$chatdata['lastcid'] = $chat['cid'];}
		$chat['msg'] = htmlspecialchars($chat['msg']);
		$t=substr($chat['send'],strpos($chat['send'],'】')+3);	//UTF8编码中这个见鬼的字符占3个字节
		$msg = "{$t}：{$chat['msg']}<br>";
		$chatdata['msg'][$chat['cid']] = $msg;
	}
	return $chatdata;
