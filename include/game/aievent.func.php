...
function aievent($rate){
	global $log,$now,$plsinfo,$db,$tablepre,$gamecfg,$gamevars,$validnum,$chatlimit,$ai_minplayer;
	global $validtime,$killnum;
	
	include_once config('aicfg',$gamecfg);
...
//		$db->query("INSERT INTO {$tablepre}chat (type,`time`,send,recv,msg) VALUES ('2','$now','SANMA_TK','$splsinfo','……静流已抵达{$splsinfo}并待机。')");
		if($sdata['hp'] <= 0){//若静流已死则中止循环，更新标签
			...
			save_gameinfo();
		}	elseif($validnum > $ai_minplayer){//AI在激活人数多于x人时才开始运行
			...
			if($sdata['hp'] <= $sdata['mhp'] * 0.75){//AI优先补血
				...
				$curenum ++;
				if(rand(0,9) < $curenum){//AI被打伤时有概率给自己上BUFF
					$curenum = 0;
					...
					foreach(array('wp','wk','wg','wc','wd','wf') as $val){
						$sdata[$val] = round($sdata[$val]*1.05);//全系熟练度+5%
					}
				...
				$mvflg = $cmbtflg = false;
				//AI判断改为玩家RP的排名与玩家RP与时间的比值共同决定
				$rp_sum = 0;
				foreach($rplist as $rpval){
					$rp_sum += $rpval['rp'];
				}
				$my_rp_rate = $rp / ($now - $validtime);
				$rp_safe_rate = 900 / 900;//安全值，被除数900意味着45个兵，除数900意味着15分钟之内做到
				$rp_mv_rate = 80;//满足条件时被跟踪的概率系数
				$rp_cmbt_rate = 80;//满足条件而且被跟踪，被直接切入战斗的概率系数
				$baserate = ($my_rp_rate - $rp_safe_rate) / $rp_safe_rate; if($baserate > 1){$baserate = 1;}elseif($baserate < 0){$baserate = 0;} //基础概率，玩家RP与时间比值与安全值共同决定
				//echo $baserate.' ';
				
				$finalrate = ($rp / $rp_sum)*$baserate;//最终概率，在基础概率上乘以玩家RP在RP池里的比例
				//echo $finalrate;
				$mvrate = $finalrate * $rp_mv_rate;
				$cmbtrate = $finalrate * $rp_cmbt_rate;
//				$mvrate = round(($myrprate - $rpratesafe) / ($rpratelvl - $rpratesafe) * 80);if($mvrate > $rpmvrate){$mvrate = $rpmvrate;}elseif($mvrate < 0){$mvrate = 0;}
				//$log.= '跟踪'.$mvrate;
//				$cmbtrate = round(($myrprate - $rpratelvl) / ($rpratelvl2 - $rpratelvl) * 80);if($cmbtrate > $rpcbtrate){$cmbtrate = $rpcbtrate;}elseif($cmbtrate < 0){$cmbtrate = 0;}
				//$log.= '战斗'.$cmbtrate;
