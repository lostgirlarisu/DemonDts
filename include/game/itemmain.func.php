...
function trap(){
	global $gametype,$teamID,$log,$cmd,$mode,$iteminfo,$itm0,$itmk0,$itme0,$itms0,$itmsk0,$nick;
	global $name,$now,$hp,$sp,$msp,$db,$tablepre,$bid,$lvl,$pid,$type,$tactic,$club,$skills;
	global $wepsk,$arbsk,$arhsk,$arask,$arfsk,$artsk,$achievement,$inf;
	
	$playerflag = $itmsk0 ? true : false;
	$selflag = $itmsk0 == $pid ? true : false;
	$teamflag=false;
	if ($gametype==2 && $teamID && $playerflag && !$selflag)	//团战模式下，队友设置的地雷回避率也大幅增加
	{
		$result = $db->query("SELECT teamID FROM {$tablepre}players WHERE pid='$itmsk0'");
		if ($db->num_rows($result))
		{
			$data = $db->fetch_array($result);
			if ($data['teamID']==$teamID) $teamflag=true;
		}
	}
	$dice=rand(0,99);
	...
	if ($gametype==2) $escrate = $teamflag ? $escrate + 50 : $escrate; //自己设置的陷阱容易躲避
	//echo '回避率：'.$escrate.'%';
	...
	if($playerflag && !$selflag){
		...
		$trname = $wdata['name'];$trtype = $wdata['type'];$trclb=$wdata['club'];$trperfix = '<span class="yellow">'.$trname.'</span>设置的';
	}elseif($selflag){
		$trname = $name;$trtype = 0;$trperfix = '你自己设置的';$trclb=$club;
	}else{
		$trname = $trtype = $trperfix = $trclb = '';
	}
		
	if($dice >= $escrate){
		$bid = $itmsk0;
		
		$damage=0; $edmg=0; $pdmg=0;
		if($itmk0 == 'TOc'){//奇迹陷阱
			$damage = $hp;
			$goodmancard = 0;
		}else{
			$damage = round(rand(0,$itme0/2)+($itme0/2));
			if (strpos($itm0,'电气')!==false) $edmg=round($itme0*0.06)+20;
			if (strpos($itm0,'毒性')!==false) $pdmg=round($itme0*0.06)+20;
			if (rand(0,99)<75) $damage = $tactic == 2 ? round($damage * 0.75) : $damage;
			$edmg = $tactic == 2 ? round($edmg * 1.25) : $edmg;
			$pdmg = $tactic == 2 ? round($pdmg * 1.25) : $pdmg;
			if ($trclb==97) $damage=round($damage*1.2);
			//好人卡特别活动
		...
		if(strpos($def_key,'m') !== false && $itmk0 != 'TOc'){//迎击
			...
			if($dice < $hitrate){
				$damage = 0; $edmg=0; $pdmg=0;$yj=true;
			}			
		}
		
		$sdice=0;
		//超能预感
		if (($lvl>=15)&&($club==9)&&($damage) && $itmk0 != 'TOc'){
			$sdice=round($damage*rand(0,30)/100);
			if ($sdice>=$damage) $sdice=$damage-1;
			$damage=$damage-$sdice;
			if ($sdice){
				$log .= "<span class=\"clan\">你感到一阵不安，于是迅速的护住了脸……</span><br>";
			}
		}
		//
		if ($club==23 && $itmk0 != 'TOc'){
			$tqd=$sp-1;
			if ($tqd<0) $tqd=0;
			if ($tqd>$damage) $tqd=$damage;
			$damage=$damage-$tqd;
			$sp=$sp-$tqd;
			if ($tqd){
				$log .= "<span class=\"clan\">你看着面前的陷阱，摆出了防御架势……</span><br>";
			}
		}
		if($damage){
			...
			if($playerflag){
				addnews($now,'trap',$nick.' '.$name,$trname,$itm0,$damage);
			}
			
			$log .= "糟糕，你触发了{$trperfix}陷阱<span class=\"yellow\">$itm0</span>！受到<span class=\"dmg\">$damage</span>点伤害！<br>";
			if ($tqd){
				$log .= "你消耗了<span class=\"yellow\">$tqd</span>点体力，抵挡了相同数值的伤害！<br>";
			}
			$gm = 0;
			if($goodmancard){
			...
			if ($pdmg)
			{
				$log .= "毒性攻击造成了<span class=\"red\">$pdmg</span>点额外伤害，并造成你<span class=\"purple\">中毒</span>了！<br>";
				$hp -= $pdmg; $inf = str_replace('p','',$inf); $inf = ($inf . 'p');
			}
			if ($edmg)
			{
				$log .= "电击造成了<span class=\"red\">$edmg</span>点额外伤害，并造成你<span class=\"yellow\">全身麻痹</span>了！<br>";
				$hp -= $edmg; $inf = str_replace('e','',$inf); $inf = ($inf . 'e');
			}
			if ($pdmg || $edmg || $gm)
			{
				$tdmg=$damage+$pdmg+$edmg+$gm;
				$log .= "<span class=\"yellow\">造成的总伤害：</span><span class=\"red\">$tdmg</span><span class=\"yellow\">。</span><br>";
			}
			$trapkill=false;
			if($hp <= 0) {
				...
				$result = $db->query("SELECT type,killnum FROM {$tablepre}players WHERE pid='$itmsk0'");
				if ($db->num_rows($result))
				{
					$data = $db->fetch_array($result);
					$new_knum=$data['killnum']+1;
					if ($data['type']==0)
					{
						$db->query ( "UPDATE {$tablepre}players SET killnum=$new_knum WHERE pid='$itmsk0'" );
					}
				}
			...
			else
			{
				...
				if (($sdice)>=$hp){
					$log .= "<span class=\"clan\">还好你机智的护住了脸，否则就被陷阱杀死了！</span><br>";
				}
			}
			...
			if ((!$yj)&&($club==23)){
				addnews($now,'trapdef1',$nick.' '.$name,$trname,$itm0);
				if ($tqd){
					$log .= "你消耗了<span class=\"yellow\">$tqd</span>点体力，抵挡了相同数值的伤害！<br>";
				}
				$log .= "你触发了{$trperfix}陷阱<span class=\"yellow\">$itm0</span>，<br><span class=\"yellow\">但是这种垃圾对拳法家是没有用的！</span><br>";
			}else{
...
function itemoff($item){
	global $log,$mode,$cmd,$itm0,$itmk0,$itme0,$itms0,$itmsk0,$wepexp;
...
function getcorpse($item){
	...
	global $itm0,$itmk0,$itme0,$itms0,$itmsk0,$money,$pls,$action,$club,$lvl;
	...
	if($item == 'wep') {
	...
	} elseif(strpos($item,'itembag')!==false){
		$iid = substr($item,8);
		$wilist = json_decode($edata['itembag'],true);
		$itm0 = $wilist[$iid]['itm'];
		$itmk0 = $wilist[$iid]['itmk'];
		$itme0 = $wilist[$iid]['itme'];
		$itms0 = $wilist[$iid]['itms'];
		$itmsk0 = $wilist[$iid]['itmsk'];
		unset($wilist[$iid]);
		$edata['itembag'] = json_encode($wilist,JSON_UNESCAPED_UNICODE);
	} elseif(strpos($item,'ar') === 0) {
		$itm0 = $edata[$item];
		$itmk0 = $edata[$item.'k'];
		$itme0 = $edata[$item.'e'];
		...
		$mmm= 0;
		if (($lvl>=7)&&($club==4)) {
			$mmm= round($edata['money']*$lvl/100);
		}
		$money = $money+$edata['money']+$mmm;
		$log .= '获得了金钱 <span class="yellow">'.$edata['money'].'</span>。<br>';
		if ($mmm){
			$log .= "掠夺技能使你获得了额外的<span class=\"yellow\">{$mmm}</span>元。<br>";
		}
		$edata['money'] = 0;
		player_save($edata);
		$action = '';
