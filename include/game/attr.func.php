...
function get_find_r($weather = 0,$pls = 0,$pose = 0,$tactic = 0,$club = 0,$inf = '',$lv=0){
	$_FIND = Array
		(
		...
		'pose' => array(0,0,0,25,-10,-25,0),
		'tactic' => array(),
		);
	$find_r = 0;
	$find_r += $_FIND['pose'][$pose];
	$wr=$_FIND['weather'][$weather];
	if (($club==6)&&($lv>=3)) $wr=abs($wr);
	$find_r += $wr;
...
//先攻几率修正，越大越容易先攻                                                            
function get_active_r($weather = 0,$pls = 0,$pose = 0,$tactic = 0,$club = 0,$inf = '',$wpose = 0,$lv){
	...
	$_ACTIVE = Array
		...
		'pls' => array(),
		'pose' => array(0,0,0,0,25,-25,0),
		'tactic' => array(),
		);
	$_DACTIVE= Array
		(
		'pose' => array(0,0,60,0,0,0,60),
		);
	$active_r = $active_obbs;
	$wr = $_ACTIVE['weather'][$weather];
	if (($lv>=3)&&($club==6)) $wr=abs($wr);
	$active_r += $wr;
...
//攻击力修正，百分比增加
function get_attack_p($weather = 0,$pls = 0,$pose = 0,$tactic = 0,$club = 0,$inf = '',$active = 1,$lv){
	global $inf_att_p;
	$_ATTACK = Array
		(
		'weather' => array(10,10,0,-5,-10,-20,-15,0,0,7,20,-7,-20,-5,-10,-10,-10),
		'pls' => array(0,0,0,0,0,0,10,0,0,-10,0,0,0,0,-10,0,0,0,10,0,0,0),
		'pose' => array(0,100,0,-25,25,-50,0),
		'tactic' => array(0,20,0,25,-50), //防御不再减攻击
		);

	$attack = 100;
	$wr=$_ATTACK['weather'][$weather];
	if (($lv>=3)&&($club==6)) $wr=abs($wr);
	$attack += $wr;
