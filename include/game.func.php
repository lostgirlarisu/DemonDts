...
function init_profile(){
	...
	global $ss,$mss;//[u150914]偶像大师技能相关
...
//	}
	...
	$hpimg='red2.gif';
	if ((strpos($inf,'B') !== false)) $hpimg='red-B.gif';
	$newhppre = 5+floor(151*(1-$hp/$mhp));
	$newhpimg = '<img src="img/'.$hpimg.'" style="position:absolute; clip:rect('.$newhppre.'px,55px,160px,0px);">';
...
function init_battle($ismeet = 0){
	global $w_type,$w_club,$w_name,$w_gd,$w_sNo,$w_icon,$w_lvl,$w_rage,$w_hp,$w_sp,$w_mhp,$w_msp,$w_wep,$w_wepk,$w_wepe,$w_sNoinfo,$w_iconImg,$w_hpstate,$w_spstate,$w_ragestate,$w_wepestate,$w_isdead,$hpinfo,$spinfo,$rageinfo,$wepeinfo,$fog,$typeinfo,$sexinfo,$infinfo,$w_exp,$w_upexp,$baseexp,$w_pose,$w_tactic,$w_inf,$w_infdata,$club,$lvl;
	global $ss,$mss;//[u150914]偶像大师技能相关
	...
	if($w_hp <= 0) {
		...
		if (($club==6)&&($lvl>=11)){
			if($w_hp < $w_mhp*0.2) {
				$w_hpstate="<span class=\"red\">$w_hp / $w_mhp</span>";
			} elseif($w_hp < $w_mhp*0.5) {
				$w_hpstate="<span class=\"yellow\">$w_hp / $w_mhp</span>";
			} else {
				$w_hpstate="<span class=\"clan\">$w_hp / $w_mhp</span>";
			}
			$w_spstate="$w_sp / $w_msp";
			if($w_rage >= 70) {
				$w_ragestate="<span class=\"red\">$w_rage</span>";
			} elseif($w_rage >= 40) {	
				$w_ragestate="<span class=\"yellow\">$w_rage</span>";
			} else {
				$w_ragestate="$w_rage";
			}
		}
