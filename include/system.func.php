...
function rs_game($mode = 0) {
	global $db,$tablepre,$gamecfg,$now,$gamestate,$gamevars,$plsinfo,$typeinfo,$areanum,$areaadd,$afktime,$combonum,$deathlimit;
	...
	if ($mode & 1) {
		...
		//重设游戏剧情开关
		$gamevars = Array('sanmaact' => 0, 'sanmadead' => 0);
		save_gameinfo();
	...
	if ($mode & 8) {
		...
		foreach ($npcinfo as $i => $npcs){
			if(!empty($npcs)) {
				if ($i==21){
					if (rand(1,1000)<=500){
						$npcs['sub'][1]=$npcs['sub'][0];
					}
					array_shift($npcs['sub']);
					shuffle($npcs['sub']);
				}
				...
				for($j = 1; $j <= $npcs['num']; $j++) {
					...
					$npc['mhp']*=get_teamfight_npchp_multipler();
					$npc['hp'] = $npc['mhp'];
					$npc['sp'] = $npc['msp'];
...
function add_once_area($atime) {
	...
	if (($gamestate > 10)&&($now > $atime)) {
		...
		if(($areanum >= $arealimit*$areaadd)&&($validnum<=0)) {//无人参加GAMEOVER不是因为这里，这里只是保险。
			...
			$areanum = $plsnum;
			if ($weather!=14 && $weather!=15 && $weather!=16) $weather = rand(0,9); else $weather=rand(14,16);
			...
			while($sub = $db->fetch_array($query)) {
				...
				} elseif($sub['type'] != 1 && $sub['type'] != 2 && $sub['type'] != 13 && $sub['type'] != 17) {
					do{$pls = $arealist[rand($areanum+1,$plsnum)];}while ($pls==34);
					$db->query("UPDATE {$tablepre}players SET pls='$pls' WHERE pid=$pid");
				}
			}
...
function gameover($time = 0, $mode = '', $winname = '') {
	global $gametype,$gamestate,$gametype,$winmode,$alivenum,$winner,$now,$gamenum,$db,$tablepre,$gamenum,$starttime,$validnum,$hdamage,$hplayer,$server_addr,$cache_server_addr,$is_cache_server;
	...
	if((!$mode)||(($mode==2)&&(!$winname))) {//在没提供游戏结束模式的情况下，自行判断模式
		...
		$winmode = substr($mode,3,2);//[u151007]结局编号上升到两位数
		$winner = $winname;
	}
	$time = $time ? $time : $now;
	...
	if($winmode == 4){//无人参加；不需要记录任何资料
		$getime = $time;
		$db->query("INSERT INTO {$tablepre}winners (gid,gametype,wmode,vnum,getime) VALUES ('$gamenum','$gametype','$winmode','$validnum','$getime')");
	}	
	elseif(($winmode == 0)||($winmode == 1)||($winmode == 6)){//程序故障、全部死亡、GM中止，不需要记录优胜者资料
		...
		$hkill = $hk['killnum'];
		$hkp = $hk['name'];
		$db->query("INSERT INTO {$tablepre}winners (gid,gametype,wmode,vnum,gtime,gstime,getime,hdmg,hdp,hkill,hkp) VALUES ('$gamenum','$gametype','$winmode','$validnum','$gtime','$gstime','$getime','$hdamage','$hplayer','$hkill','$hkp')");
	} elseif ($winmode==2 && $gametype==2) {	//团战优胜者列表
		$gstime = $starttime;
		$getime = $time;
		$gtime = $time - $starttime;
		$result = $db->query("SELECT name,killnum FROM {$tablepre}players WHERE type=0 order by killnum desc, lvl desc limit 1");
		...
		$hkill = $hk['killnum'];
		$hkp = $hk['name'];
		$db->query("INSERT INTO {$tablepre}winners (gid,gametype,wmode,vnum,gtime,gstime,getime,hdmg,hdp,hkill,hkp,winnum,namelist,gdlist,iconlist,weplist,teamID) VALUES ('$gamenum','$gametype','$winmode','$validnum','$gtime','$gstime','$getime','$hdamage','$hplayer','$hkill','$hkp','$winnum','$namelist','$gdlist','$iconlist','$weplist','$firstteamID')");
	...
	if ($gametype==2 && $winmode==2)
	{
		$namel=substr($namelist,0,strlen($namelist)-1);
		if ($firstteamID)
			addnews($time, "end$winmode","队伍<span class=\"lime\">$firstteamID</span>，成员为$namel");
		else  addnews($time, "end$winmode",$namel);
	}
	else  addnews($time, "end$winmode",$winner);
	//addnews($time, 'gameover',$gamenum);
	addnews($time, 'gameover' ,$gamenum);
	systemputchat($time,'gameover');
	if ($server_addr==$cache_server_addr || !$is_cache_server)	//高速服务器不写文件，写文件留给低速服务器上的daemon完成
	{
...
function addnpc($type,$sub,$num,$owner = 'bzk') { //foo 
	...
	if(!$npc){
		...
		for($i=0;$i< $num;$i++){
			...
			if ($owner!='bzk'){
				$npc['art']=$owner."的契约书";
				$result = $db->query("SELECT pls FROM {$tablepre}players WHERE name = '$owner' AND type=0");
				$ps = $db->result($result, 0);
				$npc['pls'] = $ps;
			}
			$npc['type'] = $type;
			$npc['endtime'] = 0;
...
function set_credits(){
	...
	foreach($clist as $key => $val){
		...
		$credits2 = $key == $winner ? $val['credits2'] + 25 : $val['credits2'] + 5;
...
//	$maxskill = ;
	...
	$up += round($data['money']/500) > 500 ? 500 : round($data['money']/500);//每500点金钱加1，顶多500
...
//				}
				...
				$gblog .= '无判断正确者，奖池的50%归获胜者。';
				$wcrup = ceil($bpool * 0.5);
				$bwlist[] = Array('uname' => '获胜者', 'wager' => '', 'bname' => '', 'odds' => '', 'crup' => $wcrup, 'crrst' => $wcrup);
				if(is_array($updatelist) && isset($updatelist[$winner]['credits2'])){
					$updatelist[$winner]['credits2'] += $wcrup;
				}else{
