...
$botservice_pagestartime=microtime();

$result = $db->query("SELECT * FROM {$tablepre}players WHERE name = '$cuser' AND type = 0");

$botservice_pageendtime = microtime();
$botservice_starttime = explode(" ",$botservice_pagestartime);
$botservice_endtime = explode(" ",$botservice_pageendtime);
$botservice_totaltime = $botservice_endtime[0]-$botservice_starttime[0]+$botservice_endtime[1]-$botservice_starttime[1];
$botservice_timecost = sprintf("%s",$botservice_totaltime);
echo "verify_time=$botservice_timecost\n"; 
$botservice_pagestartime=microtime();
	
echo "botservice_version=0.3-20140219-1\n";
...
{ 
	...
	$botservice_totaltime = $botservice_endtime[0]-$botservice_starttime[0]+$botservice_endtime[1]-$botservice_starttime[1];
	$botservice_timecost = sprintf("%s",$botservice_totaltime);
	echo "verify_end_time=$botservice_timecost\n"; 
	exit(); 
}
...
if (isset($command) && (!$corpseflag || $command=='getcorpse'))
{
	...
	else  if ($command=="pvpinfo")
	{
		$result = $db->query("SELECT * FROM {$tablepre}players WHERE type = 0 AND state < 10");
		$rows=$db->num_rows($result);
		echo "pcalivenum=$rows\n";
		$i=0;
		while($data = $db->fetch_array($result)) 
		{
			$i++;
			echo "pcalivesNo{$i}={$data['sNo']}\n";
			echo "pcalivewepk{$i}={$data['wepk']}\n";
			echo "pcalivewepe{$i}={$data['wepe']}\n";
			echo "pcalivewepsk{$i}={$data['wepsk']}\n";
			$defall=$data['def'];
			foreach (Array('arb','arh','ara','arf') as $value) if ($data[$value.'s']) $defall+=$data[$value.'e'];
			echo "pcalivedefall{$i}=$defall\n";
			echo "pcalivemhp{$i}={$data['mhp']}\n";
			$skill=0;
			foreach (Array('wp','wk','wc','wg','wd','wf') as $value) $skill=max($skill,$data[$value]);
			echo "pcaliveskill{$i}={$skill}\n";
			if ($data['teamID']==$teamID && $teamID)
				echo "pcaliveisteammate{$i}=1\n";
			else  echo "pcaliveisteammate{$i}=0\n";
		}
	}
}
...
foreach (Array('sNo','mhp','hp','msp','sp','rage','money','club','inf','mss','ss','skillpoint','att','def','pls','lvl','pose','tactic','wp','wk','wc','wg','wd','wf','action') as $value) 
	echo "{$value}=".${$value}."\n";
...
$result = $db->query("SELECT type,sNo,pls,name,state,bid,deathtime FROM {$tablepre}players WHERE type > 0 AND deathtime >= $endtime");
$rows=$db->num_rows($result);
...
{
	$i++;
	echo "deathnpctime$i={$data['deathtime']}\n";
...
$result = $db->query("SELECT sNo,pls,name,state,bid,deathtime FROM {$tablepre}players WHERE type = 0 AND state >= 10 AND deathtime >= $endtime");
$rows=$db->num_rows($result);
...
{
	$i++;
	echo "deathpctime$i={$data['deathtime']}\n";
...
$result = $db->query("SELECT * FROM {$tablepre}newsinfo WHERE time >= $endtime ORDER BY nid");
$rows=$db->num_rows($result);
echo "newsnum=$rows\n";
$i=0;
while($data = $db->fetch_array($result)) 
{
	$i++;
	echo "newsnid{$i}={$data['nid']}\n";
	echo "newstime{$i}={$data['time']}\n";
	echo "newstype{$i}={$data['news']}\n";
	echo "newsinfoa{$i}={$data['a']}\n";
	echo "newsinfob{$i}={$data['b']}\n";
	echo "newsinfoc{$i}={$data['c']}\n";
	echo "newsinfod{$i}={$data['d']}\n";
	echo "newsinfoe{$i}={$data['e']}\n";
}
